...
import type { NextPage } from 'next';
import NFTCard from '~~/components/simpleNFT/NFTCard'; // 导入 NFTCard 组件
import { useAccount } from 'wagmi'; // 使用 wagmi 中的 useAccount hook 获取当前用户地址
import { useScaffoldContractWrite } from '~~/hooks/scaffold-eth'; // 使用 scaffold-eth 中的 useScaffoldContractWrite hook 处理合约写入
import { notification } from '~~/utils/scaffold-eth/notification'; // 使用 scaffold-eth 中的 notification 函数显示通知
import './page.css'; // 导入页面样式
...
const Home: NextPage = () => {
  ...
  const { writeAsync: purchaseAsync } = useScaffoldContractWrite({ // 获取 purchaseNFT 合约写入方法
  ...
  const filteredNFTs = nfts.filter((nft) =>
    ...
    nft.nftholder.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  return (
    <div className="flex items-center flex-col flex-grow pt-10">
      <div className="px-5 w-full max-w-3xl mx-auto">
        <h1 className="text-center mb-4 text-4xl font-bold text-gray-900">
          NFT文创展示
        </h1>
        <input
          type="text"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 gap-large">
          {visibleNFTs.length === 0 ? (
            <p className="text-center col-span-full text-lg mt-8 text-gray-700">
              没有符合搜索条件的NFT。
            </p>
          ) : (
            visibleNFTs.map((nft) => (
